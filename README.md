...
```
uv run pyinstaller --onefile --icon lightimer.ico --add-data "sound/universfield-game-level-complete-143022.mp3:sound/" --name lightimer main.py
```

and find the newly installed binary file `lightimer` (`lightimer.exe` on Windows) within the `dist/` folder.

[//]: # (Install on Windows:
pyinstaller --onefile --noconsole --icon lightimer.ico --name lightimer main.py --add-data "sound/universfield-game-level-complete-143022.mp3;sound/")
